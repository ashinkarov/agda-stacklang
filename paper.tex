...
\begin{abstract}
  This paper proposes a novel tecnhique to write correct by construction
  programs in languages that do not have built-in verification
  capabilities.  This is achieved in three steps.  Firstly, we
  shallowly embed the language (or its subset) into a
  ...
  The main insight of the paper is the observation that de\-pen\-den\-tly-typed
  languages that are equipped with reflection mecahnisms make it possible
  to expres all the three steps in a single language.  Essentially,
  this allows us to express programs, its formal properties and
  compilers hand-in-hand, offering a lot of flexibility to programmers.

  To demonstrate the approach precisely, we embed a subset of
  the PostScript language in Agda.  We explain all the three
  steps, illustrating them with several short examples.
