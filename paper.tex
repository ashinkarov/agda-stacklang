...
When writing a program, it is often desirable to guarantee that a
certain class of errors does not occur in the given program.  For
example, the program can never dereference a null pointer; or division
by zero never occurs; or indexing is always within bounds.  In most
programming languages, the class of guarantees that can be checked
statically is very limited.  Nevertheless, moving to a different
language is not always an option.  For example, these languages may
come with great tooling, compilers and libraries.  In this paper, we
...
one can also embed the language we want to use into an existing
dependently typed language, which allows us to re-use the typechecker
...
defining a tagless interpreter is technically possible~\cite{}, but
extremely challenging to define and use in practice.

We propose a novel approach that combines the best parts of shallow
and deep embeddings through the use of reflection in a
dependently-typed host language such as Coq, Agda, Idris, or Lean.
Concretely, we start by defining a shallow embedding of the target
language while making use of dependent types to statically enforce the
properties of interest, and write a program in this embedding. We then
make use of reflection to obtain a syntactic representation of the
program, and extract a program in the syntax of the target language
from it.  Finally, we compile the extracted program using existing
tools and libraries. Essentially, a shallow embedding in a host
language with reflections suddenly becomes deep.
...
language used in the corresponding document format.  The simplicity of the language
...
\item We demonstate this approach by a concrete example: an embedding
...
\section{Scaling to Bigger Languages} \label{sec:scaling}
