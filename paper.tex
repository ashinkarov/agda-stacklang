...
\begin{abstract}
  ...
  In this paper we explore a way to write correct by construction
  programs in languages that do not have built-in verification
  capabilities.  This is achieved in three steps.  Firstly, we
  shallowly embed the language (or its subset) inside of a
  dependently-typed system.  Secondly, we write a program
  within that embedding using dependent types to guarantee
  any property of interest.  Thirdly, we provide a way to
  translate the embedded program back into the original language,
  \eg{} so that it can be used by the existing compilers.

  The main focus of the paper is the third step, which we
  call \emph{extraction}.  The main insight of the paper is
  the use of reflection mechanism of a dependently-typed language
  to achieve extraction.  As a result, the embedding, the
  program and the extractor are written in a single language.

  To demonstrate these ideas precisely we embed a subset of
  the PostScript language in Agda and explain all the three
  steps.
